...
class TestModel1(unittest.TestCase):
    ...
    def test_check_bhat(self):
        np.testing.assert_allclose(
            self.Xty1, 
            (self.N1 - 1) * get_diag_as_vec(self.Rhat1) * self.bhat1
        )
    def test_init(self):
        mod = Model1(
        ...
        b = (self.N1 - 1) * np.diagonal(self.Rhat1.toarray()) * self.bhat1 + X2.T @ self.y2
    ...
    def test_solve_cmp_w_sklearn(self):
        maxiter = 1000
        tol = 1e-6
        ...
        self.assertTrue(obj_model1(beta_sk, mod.A, mod.b, w1, w2) >= obj_model1(beta_my, mod.A, mod.b, w1, w2))
